...
def render_blueprint(preview: BlueprintPreview, saved=None, message: str | None = None) -> str:
    ...
    def value(name: str) -> str:
        return escape(str(saved[name])) if saved and saved[name] is not None else ""
    form = ""
    if preview.unlocked:
        form = f"""<section class="unknown"><h2>Blueprint formu</h2><form method="post" action="/blueprint/save">
        <label>Hedef kitle<input name="audience" required value="{value('audience')}"></label>
        <label>Müşteri problemi<textarea name="customer_problem" required>{value('customer_problem')}</textarea></label>
        <label>Ürün vaadi<textarea name="product_promise" required>{value('product_promise')}</textarea></label>
        <div class="numbers"><label>Min. yaş<input type="number" min="0" name="age_min" required value="{value('age_min')}"></label><label>Maks. yaş<input type="number" min="0" name="age_max" required value="{value('age_max')}"></label><label>Sayfa<input type="number" min="1" name="page_count" required value="{value('page_count')}"></label><label>Aktivite<input type="number" min="1" name="activity_count" required value="{value('activity_count')}"></label></div>
        <label>Özgün farklılaştırıcı<textarea name="differentiator" required>{value('differentiator')}</textarea></label>
        <div class="numbers"><label>Hedef fiyat<input type="number" min="0.01" step="0.01" name="target_price" required value="{value('target_price')}"></label><label>Para birimi<input name="currency" required value="{value('currency')}"></label></div>
        <label>İçerik yapısı<textarea name="content_structure" required>{value('content_structure')}</textarea></label>
        <label class="confirm"><input type="checkbox" name="confirm" value="YES" required> Bu planı insan kararı olarak kaydet.</label><button>KAYDET</button></form></section>"""
...
<style>body{{margin:0;background:#14231d;color:#f7f2e8;font-family:Segoe UI,Arial,sans-serif}}main{{width:min(850px,92vw);margin:50px auto}}a{{color:#d9f06d}}.eyebrow{{color:#d9f06d;font-size:11px;letter-spacing:.18em;font-weight:800}}h1{{font-size:52px;letter-spacing:-.055em;margin:12px 0}}.state{{padding:15px 18px;border:1px solid #60746a;border-radius:14px;background:#20342b}}.notice{{margin:16px 0;padding:14px;background:#d9f06d;color:#14231d;border-radius:12px}}.known,.unknown{{margin-top:22px;background:#fffdf8;color:#14231d;padding:24px;border-radius:20px}}dl{{display:grid;grid-template-columns:160px 1fr;gap:12px}}dt{{color:#66756e}}dd{{margin:0;font-weight:700}}ul{{list-style:none;padding:0}}li{{display:flex;justify-content:space-between;padding:11px 0;border-bottom:1px solid #e7e2d8}}li b{{color:#a43d32;font-size:12px}}label{{display:block;margin:13px 0;color:#526158}}input,textarea{{display:block;width:100%;padding:11px;margin-top:5px;border:1px solid #cbc8be;border-radius:10px;font:inherit}}textarea{{min-height:70px}}.numbers{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}}.confirm input{{display:inline;width:auto}}button{{padding:13px 22px;border:0;border-radius:99px;background:#186b4b;color:white;font-weight:800}}@media(max-width:650px){{.numbers{{grid-template-columns:1fr 1fr}}}}</style></head><body><main><a href="/review">← Fırsat incelemesi</a>{notice}<p class="eyebrow">PRODUCT BLUEPRINT</p><h1>{escape(preview.title)}</h1><div class="state">{state}</div><section class="known"><h2>Kanıtla bilinenler</h2><dl><dt>Kanal</dt><dd>{escape(preview.channel)}</dd><dt>Ürün tipi</dt><dd>{escape(preview.product_type)}</dd><dt>Niş</dt><dd>{escape(preview.niche)}</dd><dt>Gözlenen fiyat</dt><dd>{escape(preview.observed_price_range)}</dd></dl></section>{form if preview.unlocked else f'<section class="unknown"><h2>İnsan kararı gereken alanlar</h2><ul>{unknowns}</ul></section>'}</main></body></html>"""
